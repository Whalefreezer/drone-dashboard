...
Deno.test("Sort Leaderboard - Pilots Without Laps Tiebreaker (Races Until Next > Channel)", () => {
  const pilotC_WaitingSooner = createEntry("p3", "Charlie", { racesUntilNext: 2, channel: mockChannel("c3", 3) });
  const pilotJ_WaitingLater = createEntry("p10", "Judy", { racesUntilNext: 5, channel: mockChannel("c10", 10) });
  const pilotK_NotWaiting_LowChan = createEntry("p11", "Kevin", { racesUntilNext: -1, channel: mockChannel("c11", 1) });
  const pilotL_NotWaiting_HighChan = createEntry("p12", "Liam", { racesUntilNext: -1, channel: mockChannel("c12", 12) });

  const entries = [pilotL_NotWaiting_HighChan, pilotJ_WaitingLater, pilotC_WaitingSooner, pilotK_NotWaiting_LowChan];
  const sorted = sortLeaderboard(entries, defaultLeaderboardSortConfig);
  const sortedNames = sorted.map(e => e.pilot.Name);
...
Deno.test("Sort Leaderboard - Eliminated Pilots Order (Stage > Points)", () => {
  const entries = [pilotG_Eliminated_EarlierStage, pilotF_Eliminated_LowPoints, pilotE_Eliminated_HighPoints];
  const sorted = sortLeaderboard(entries, defaultLeaderboardSortConfig);
  const sortedNames = sorted.map(e => e.pilot.Name);
...
Deno.test("Sort Leaderboard - Default Sort (Catch-all)", () => {
    ...
    const pilotM = createEntry("p13", "Mallory", { racesUntilNext: 1, channel: mockChannel("c13", 5) }); // No laps, waiting
    const pilotN = createEntry("p14", "Nate", { racesUntilNext: -1, channel: mockChannel("c14", 2) }); // No laps, not waiting
