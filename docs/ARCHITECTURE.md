...
| `client_kv`                                                             | Backend-published race order + admin KV (leaderboard splits/overrides, locked elimination rankings, elimination format+anchors config, stream links) | `backend/scheduler/race.go`, `frontend/src/routes/admin/kv.tsx`, `frontend/src/bracket/eliminationState.ts` |
