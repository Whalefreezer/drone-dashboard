...
func (s *Service) IngestRace(eventId, raceId string) error {
    ...
    for _, pc := range r.PilotChannels {
        ...
        // compose a stable sourceId for pilotChannels: raceId+pctuple
        pcSource := fmt.Sprintf("%s:%s:%s", r.ID, pc.Pilot, pc.Channel)
        if _, err := s.Upserter.Upsert("pilotChannels", pcSource, map[string]any{
            "pilot": pilotPBID,
            "channel": channelPBID,
            "event": eventPBID,
        }); err != nil {
