...
func (d DirectSource) FetchEvent(eventSourceId string) (EventFile, error)        { return d.C.FetchEvent(eventSourceId) }
func (d DirectSource) FetchPilots(eventSourceId string) (PilotsFile, error)      { return d.C.FetchPilots(eventSourceId) }
func (d DirectSource) FetchChannels() (ChannelsFile, error)                       { return d.C.FetchChannels() }
func (d DirectSource) FetchRounds(eventSourceId string) (RoundsFile, error)      { return d.C.FetchRounds(eventSourceId) }
func (d DirectSource) FetchRace(eventSourceId, raceId string) (RaceFile, error)  { return d.C.FetchRace(eventSourceId, raceId) }
func (d DirectSource) FetchResults(eventSourceId string) (ResultsFile, error)    { return d.C.FetchResults(eventSourceId) }
...
type cached struct{ etag string; body []byte }
...
func (r *RemoteSource) fetchJSON(path string, out any) error {
    ...
    if status == http.StatusNotModified {
        if c, ok := r.cache[path]; ok { body = c.body } else { return fmt.Errorf("304 but no cache for %s", path) }
    }
...
func (r *RemoteSource) FetchEvent(eventSourceId string) (EventFile, error)        { var o EventFile; err := r.fetchJSON("/events/"+eventSourceId+"/Event.json", &o); return o, err }
func (r *RemoteSource) FetchPilots(eventSourceId string) (PilotsFile, error)      { var o PilotsFile; err := r.fetchJSON("/events/"+eventSourceId+"/Pilots.json", &o); return o, err }
func (r *RemoteSource) FetchChannels() (ChannelsFile, error)                       { var o ChannelsFile; err := r.fetchJSON("/httpfiles/Channels.json", &o); return o, err }
func (r *RemoteSource) FetchRounds(eventSourceId string) (RoundsFile, error)      { var o RoundsFile; err := r.fetchJSON("/events/"+eventSourceId+"/Rounds.json", &o); return o, err }
func (r *RemoteSource) FetchRace(eventSourceId, raceId string) (RaceFile, error)  { var o RaceFile; err := r.fetchJSON("/events/"+eventSourceId+"/"+raceId+"/Race.json", &o); return o, err }
func (r *RemoteSource) FetchResults(eventSourceId string) (ResultsFile, error)    { var o ResultsFile; err := r.fetchJSON("/events/"+eventSourceId+"/Results.json", &o); return o, err }
