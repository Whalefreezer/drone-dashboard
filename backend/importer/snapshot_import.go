...
func ImportFromFile(app core.App, path string) error {
	...
	if snap.Version == "" || snap.Version[:12] != "pb-snapshot@" {
		slog.Warn("import.snapshot.version.unexpected", "version", snap.Version)
	}

	counts := map[string]int{}
	// Import in referential order
	if n, err := importCollection(app, "events", snap.Collections.Events); err != nil { return err } else { counts["events"] = n }
	if n, err := importCollection(app, "channels", snap.Collections.Channels); err != nil { return err } else { counts["channels"] = n }
	if n, err := importCollection(app, "pilots", snap.Collections.Pilots); err != nil { return err } else { counts["pilots"] = n }
	if n, err := importCollection(app, "rounds", snap.Collections.Rounds); err != nil { return err } else { counts["rounds"] = n }
	if n, err := importCollection(app, "races", snap.Collections.Races); err != nil { return err } else { counts["races"] = n }
	if n, err := importCollection(app, "pilotChannels", snap.Collections.PilotChannels); err != nil { return err } else { counts["pilotChannels"] = n }
	if n, err := importCollection(app, "detections", snap.Collections.Detections); err != nil { return err } else { counts["detections"] = n }
	if n, err := importCollection(app, "laps", snap.Collections.Laps); err != nil { return err } else { counts["laps"] = n }
	if n, err := importCollection(app, "gamePoints", snap.Collections.GamePoints); err != nil { return err } else { counts["gamePoints"] = n }
	if n, err := importCollection(app, "client_kv", snap.Collections.ClientKV); err != nil { return err } else { counts["client_kv"] = n }
	if n, err := importCollection(app, "ingest_targets", snap.Collections.IngestTargets); err != nil { return err } else { counts["ingest_targets"] = n }
	if n, err := importCollection(app, "server_settings", snap.Collections.ServerSettings); err != nil { return err } else { counts["server_settings"] = n }
...
func setCurrentEvent(app core.App, id string) error {
	...
	if err == nil && target != nil {
		...
		if err := app.Save(target); err != nil { return err }
	}
