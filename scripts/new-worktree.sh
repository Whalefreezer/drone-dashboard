...
# Creates a git worktree at ./.trees/NAME checked out to branch feat/NAME
# and copies any existing .env files from the repo into the worktree.

if [[ ${1-} == "" || ${1-} == "-h" || ${1-} == "--help" ]]; then
	printf "Usage: %s <name>\n" "$(basename "$0")"
	printf "Creates ./.trees/<name> worktree on branch feat/<name> and copies .env files.\n"
	exit 1
fi
...
# Ensure ./.trees exists and is writable
if [[ ! -d "./.trees" ]]; then
	if ! mkdir -p ./.trees 2>/dev/null; then
		echo "Error: cannot create ./.trees directory."
		exit 3
	fi
fi
if [[ ! -w "./.trees" ]]; then
	echo "Error: ./.trees is not writable."
	exit 4
fi
